...
export async function updateDispute(id, updates) {
  if (IS_PG) {
    const fields = Object.keys(updates);
    ...
    const rows = await pgQuery(
      `UPDATE disputes SET ${setClauses.join(', ')} WHERE id = $${fields.length + 1} RETURNING *`,
      values
    );
    return rows[0] || null;
  }
  const idx = mem.disputes.findIndex(d => d.id === id);
  if (idx === -1) return null;
  mem.disputes[idx] = { ...mem.disputes[idx], ...updates };
  return mem.disputes[idx];
}
...
export async function createSupplier(data) {
  if (IS_PG) {
    ...
    const placeholders = cols.map((_, i) => `$${i + 1}`);
    const rows = await pgQuery(
      `INSERT INTO suppliers (${cols.join(', ')}, created_at) VALUES (${placeholders.join(', ')}, NOW()) RETURNING *`,
      cols.map(k => data[k])
    );
    return rows[0];
  }
  const supplier = { ...data, id: crypto.randomUUID(), created_at: new Date().toISOString() };
...
export async function getSupplierById(id) {
  if (IS_PG) {
    const rows = await pgQuery('SELECT * FROM suppliers WHERE id = $1', [id]);
    return rows[0] || null;
  }
...
export async function updateSupplier(id, updates) {
  if (IS_PG) {
    ...
    const values = [...Object.values(updates), id];
    const rows = await pgQuery(
      `UPDATE suppliers SET ${setClauses.join(', ')} WHERE id = $${fields.length + 1} RETURNING *`,
      values
    );
    return rows[0] || null;
  }
  const idx = mem.suppliers.findIndex(s => s.id === id);
  if (idx === -1) return null;
  mem.suppliers[idx] = { ...mem.suppliers[idx], ...updates };
  return mem.suppliers[idx];
}

export async function addSupplierReview(id, rating, reviewText) {
  if (IS_PG) {
    // Atomic running-average update: new_avg = (old_avg * count + new_rating) / (count + 1)
    const rows = await pgQuery(
      `UPDATE suppliers
       SET community_rating = ROUND(
         (COALESCE(community_rating, 0) * COALESCE(review_count, 0) + $1) /
         (COALESCE(review_count, 0) + 1), 2
       ),
       review_count = COALESCE(review_count, 0) + 1
       WHERE id = $2 RETURNING id, community_rating, review_count`,
      [rating, id]
    );
    return rows[0] || null;
  }
  const idx = mem.suppliers.findIndex(s => s.id === id);
  if (idx === -1) return null;
  const s = mem.suppliers[idx];
  const count = (s.review_count || 0) + 1;
  const avg = ((s.community_rating || 0) * (count - 1) + rating) / count;
  mem.suppliers[idx] = { ...s, community_rating: Math.round(avg * 100) / 100, review_count: count };
  return { id, community_rating: mem.suppliers[idx].community_rating, review_count: count };
}
...
export async function createUser(data) {
  if (IS_PG) {
    const rows = await pgQuery(
      `INSERT INTO users (email, name, password_hash, session_token, created_at)
       VALUES ($1, $2, $3, $4, NOW()) RETURNING id, email, name, session_token, created_at`,
      [data.email.toLowerCase(), data.name, data.password_hash, data.session_token]
    );
    return rows[0];
  }
  const user = {
...
export async function getUserBySession(token) {
  ...
  if (IS_PG) {
    const rows = await pgQuery('SELECT id, email, name, created_at FROM users WHERE session_token = $1', [token]);
    return rows[0] || null;
  }
  return (mem.users || []).find(u => u.session_token === token) || null;
}

export async function updateUserSession(id, sessionToken) {
  if (IS_PG) {
    const rows = await pgQuery(
      'UPDATE users SET session_token = $1 WHERE id = $2 RETURNING id, email, name, created_at',
      [sessionToken, id]
    );
    return rows[0] || null;
  }
  const u = (mem.users || []).find(u => u.id === id);
  if (u) u.session_token = sessionToken;
  return u;
}
...
export async function createTemplate(data) {
  if (IS_PG) {
    ...
    const placeholders = cols.map((_, i) => `$${i + 1}`);
    const rows = await pgQuery(
      `INSERT INTO transfer_templates (${cols.join(', ')}, created_at)
       VALUES (${placeholders.join(', ')}, NOW()) RETURNING *`,
      cols.map(k => data[k])
    );
    return rows[0];
  }
  const tmpl = { ...data, id: crypto.randomUUID(), created_at: new Date().toISOString() };
  if (!mem.templates) mem.templates = [];
  mem.templates.unshift(tmpl);
  return tmpl;
}

export async function getTemplatesByUser(userEmail) {
  if (IS_PG) {
    return pgQuery(
      'SELECT * FROM transfer_templates WHERE user_email = $1 AND is_active = true ORDER BY created_at DESC',
      [userEmail.toLowerCase()]
    );
  }
  return (mem.templates || []).filter(t => t.user_email === userEmail.toLowerCase() && t.is_active !== false);
}

export async function getTemplateById(id) {
  if (IS_PG) {
    const rows = await pgQuery('SELECT * FROM transfer_templates WHERE id = $1', [id]);
    return rows[0] || null;
  }
  return (mem.templates || []).find(t => t.id === id) || null;
}
...
export async function updateTemplate(id, updates) {
  if (IS_PG) {
    ...
    const values = [...Object.values(updates), id];
    const rows = await pgQuery(
      `UPDATE transfer_templates SET ${setClauses.join(', ')} WHERE id = $${fields.length + 1} RETURNING *`,
      values
    );
    return rows[0] || null;
  }
  if (!mem.templates) return null;
  const idx = mem.templates.findIndex(t => t.id === id);
  if (idx === -1) return null;
  mem.templates[idx] = { ...mem.templates[idx], ...updates };
  return mem.templates[idx];
}

export async function getTemplatesWithReminders(dayOfMonth) {
  if (IS_PG) {
    return pgQuery(
      'SELECT * FROM transfer_templates WHERE reminder_enabled = true AND reminder_day = $1 AND is_active = true',
      [dayOfMonth]
    );
  }
  return (mem.templates || []).filter(
    t => t.reminder_enabled && t.reminder_day === dayOfMonth && t.is_active !== false
  );
}
